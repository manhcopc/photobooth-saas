  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-4 md:grid-cols-3">
        <StatCard helper="Mock/IndexedDB" label="Tổng events" value={loading ? '...' : events.length} />
        <StatCard helper="Ảnh final đã tạo" label="Gallery" value={loading ? '...' : images.length} />
      ...
      <section className="rounded-3xl bg-gradient-to-r from-pink-500 to-purple-700 p-6 text-white shadow-lg shadow-purple-100">
        ...
        <p className="mt-2 max-w-2xl text-white/80">Tạo event, chia sẻ QR code và xem lại ảnh final ngay trong trình duyệt bằng IndexedDB.</p>
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {events.slice(0, 3).map((event) => <EventCard event={event} imageCount={imageCountBySlug[event.slug] || 0} key={event.id} />)}
        </div>
      </section>
    </div>
  )
